...
export default async function ViewSingleItinerary({ params }) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <h1 className="text-center text-3xl mx-auto head_text">{itineraryInfo.title}</h1>
      ...
      <section className="w-full max-w-5xl flex flex-col glassmorphism my-10">
        ...
        <p className=" text-gray-700 mx-2 text-lg mb-2">
          Posted by <span className="font-bold">{itineraryInfo.username}</span> on <span className="font-bold">{formattedDate}</span>
        </p>      
        </div>
        ...
        <div className="flex justify-between mx-2">
          <div className="flex items-center">
            ...
            <p className="text-gray-700 text-lg"><span className="font-bold">{formattedBudget}</span></p>
          </div>
          <div className="flex items-center">
            <h2 className="text-lg font-semibold mr-2">Votes:</h2>
            <p className="text-lg text-gray-700"><span className="font-bold">{itineraryInfo.total_votes}</span></p>
          </div>
        </div>
      </section>
